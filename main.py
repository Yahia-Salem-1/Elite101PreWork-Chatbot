...
def money_syst():
  global money
  choice_money = input(
    'Do you want us to choose how much money you have, or do you want to do that? (choose opt1/opt2):\n'
  )
...
def new_product():
  ...
  inventory.append(
    new_product.features())  # adds new product features to the inventory
...
def order_product():
  ...
  if quantity <= avail_quantity and quantity > 0:
    ...
    if money - cost >= 0:
      ...
      choice_goBack = input(
        'Would you like to go back to display menu, or would you like to order something else? (opt1/opt2): '
      )
...
def chatbot():
  ...
  def age_func():
    try:
      ...
      if 0 <= age < 11:
        ...
        print("\nYou are in your tween/teen years! Pretty cool."
              )  # The answers to the input.
      elif 20 <= age <= 40:
  ...
  def feeling():
    try:
      day = input(
        "\nIf you had one word to describe your day, what would it be? "
      ).lower()

      good = [
        "good", "awesome", "nice", "alright", "okay", "ok", "alr", "rad",
        "cool", "best", "bright", "super", "splendid", "lovely", "exciting",
        "happy", "smooth", "pleasant", "lucky", "fine", "great"
      ]
      bad = [
        "bad", "awful", "not good", "horrible", "long", "miserable", "sad",
        "rough", "crappy", "worst", "unpleasant", "dissapointing", "dreadful",
        "difficult", "none of your business", "horrendous", "long"
      ]
      boring = [
        "meh", "eh", "boring", "repetitive", "dull", "unvaried", "in between",
        "not bad, not good", "usual"
      ]
      interesting = [
        "interesting", "fascinating", "surprising", "shocking", "weird",
        "strange", "confusing", "unusual"
      ]

      good_response = [
        "\n I agree! What a pleasant day it is.",
        "\nWhat a lovely day for both of us!", f"\nThat's {day}!",
  ...
  def recommendations():
    ...
    new_list_allAges_genE_1 = [
      inventory[index] for index in list_allAges_genE_1
    ]
    new_list_allAges_genE_2 = [
      inventory[index] for index in list_allAges_genE_2
    ]
